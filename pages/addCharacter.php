    ...
    <section id="content" class="relative w-content left-[280px] duration-300 ease-in-out max-[768px]:w-md">
        ...
        <main class="w-full py-7 px-4 font-sans">
            <div class="flex items-center gap-4 flex-wrap justify-between">
                <div>
                    <h2 class="text-4xl font-bold mb-2">Add Character</h2>
            ...
            if (isset($_GET['pesan'])) {
                ...
                if ($_GET['pesan'] == 'berhasil') {
                    ...
                    echo "<div class='p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50'>
                        <span> class='font-medium'>Success alert!</span> Character Hero Berhasil Ditambahkan.
                      </div>";
                } elseif ($_GET['pesan'] == 'gagal') {
                    # code...
                    echo "<div class='p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50'>
                    <span class='font-medium'>Danger alert!</span> Character Hero Gagal Ditambahkan.
                  </div>";
                }
            }
            ?>
            ...
            <div class="bg-white shadow-lg mt-7 max-w-xl container mx-auto w-full shadow-slate-400 rounded-md pb-5">
                <form action="cek-addCharacter.php" method="post" enctype="multipart/form-data">
                    <div class="flex flex-col px-3 py-2">
                        <input type="hidden" name="idUser" value="<?php echo $_SESSION['id']; ?>">
                        <label for="nama_hero" class="text-base text-inherit pb-2 font-medium">Nama Hero:</label>
                        <input required name="nama_hero" type="text" class="px-1 py-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring focus:ring-blue-500 md:px-2 md:py-3 md:text-base" id="" placeholder="ex: Lancelot" />
                    </div>
                    <div class="flex flex-col px-3 py-2">
                        <label for="role" class="text-base text-inherit pb-2 font-medium">Role Hero:</label>
                        <input required name="role" type="text" class="px-1 py-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring focus:ring-blue-500 md:px-2 md:py-3 md:text-base" id="" placeholder="ex: Assasin" />
                    </div>
                    <div class="flex flex-col px-3 py-2">
                        <label for="deskripsi" class="text-base text-inherit pb-2 font-medium">Deskripsi Hero:</label>
                        <input required name="deskripsi" type="text" class="px-1 py-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring focus:ring-blue-500 md:px-2 md:py-3 md:text-base" id="" placeholder="" />
                    </div>
                    <div class="flex flex-col px-3 py-2">
                        <label for="gambar" class="text-base text-inherit pb-2 font-medium">Gambar Hero:</label>
                        <input name="gambar" type="file" class="px-1 py-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring focus:ring-blue-500 md:px-2 md:py-3 md:text-base" id="" multiple />
                    </div>
                    <div class="flex flex-row px-3 py-4 pt-5 text-white font-medium">
                        <button class="bg-[#1F2937] px-4 py-2 rounded-md cursor-pointer border border-[#1F2937] transition-all duration-150 ease-in-out hover:bg-slate-600" type="submit" name="submit">
                            Tambah
                        </button>
